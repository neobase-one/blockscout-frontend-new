            ...
            <Tabs variant="outline" colorScheme="gray" size="sm" index={ tabIndex }>
              <TabList columnGap={ 3 } rowGap={ 2 } flexWrap="wrap">
                { resultCategories.map((cat, index) => (
                  <Tab key={ cat.id } onClick={ scrollToCategory(index) } _hover={{ color: 'accent', borderColor: 'accent' }}
                    _selected={{ bgColor: 'accent', color: 'white', borderColor: 'transparent' }}>{ cat.title }</Tab>
                )) }
              </TabList>
            </Tabs>
          </Box>
